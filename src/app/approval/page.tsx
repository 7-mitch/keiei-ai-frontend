...
const TYPE_LABEL: Record<string, string> = {
  jsox_document:    "J-SOX文書",
  audit_report:     "監査レポート",
  financial_report: "財務レポート",
  budget_request:   "予算申請",
  expense:          "経費申請",
  general:          "一般申請",
};
...
export default function ApprovalPage() {
  ...
  const handleApprove = async () => {
    ...
    try {
      await axios.post(
        `${API}/api/approval/requests/${selectedRequest.id}/approve`,
        { comment }, { headers: headers() }
      );
      ...
      showToast(e.response?.data?.detail || "承認に失敗しました", "error");
    }
  };
  ...
  const handleReject = async () => {
    ...
    try {
      await axios.post(
        `${API}/api/approval/requests/${selectedRequest.id}/reject`,
        { comment }, { headers: headers() }
      );
      ...
      showToast(e.response?.data?.detail || "差戻しに失敗しました", "error");
    }
  };
  ...
  const handleCreateRequest = async () => {
    ...
    try {
      ...
      showToast(e.response?.data?.detail || "申請作成に失敗しました", "error");
    }
  };
  ...
  const handleJSOX = async () => {
    ...
    try {
      const res = await axios.post(
        `${API}/api/approval/jsox/generate`,
        {
          ...
          <div className="space-y-3">
            {loading ? (
              ...
              <div className="text-center py-16 text-gray-400">
                <div className="w-12 h-12 border-2 border-dashed border-white/20 rounded-full flex items-center justify-center mx-auto mb-3">
                  <Icons.Check />
                </div>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        ...
                        <span className={`text-xs font-medium ${PRIORITY_COLOR[req.priority]}`}>
                      ...
                      <p className="text-xs text-gray-500 mt-1 flex items-center gap-2">
                        ...
                        {req.due_date && (
                          <span className="flex items-center gap-1">
                            <Icons.Clock />
                            {new Date(req.due_date).toLocaleDateString("ja-JP")}
                          </span>
                        )}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      {req.ai_risk_score !== null && (
                        <div className={`text-center px-2.5 py-1 rounded-lg text-xs min-w-[48px]
                          ${req.ai_risk_score > 0.7 ? "bg-red-500/20 text-red-300" :
                            req.ai_risk_score > 0.4 ? "bg-yellow-500/20 text-yellow-300" :
                ...
                <div className="bg-[#1a1d27] border border-white/15 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
                  <div className="p-6">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_COLOR[selectedRequest.status]} mb-2 inline-block`}>
                          ...
                          {steps.map((step, i) => (
                            <div key={step.id} className="flex items-center gap-3">
                              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0
                                ${step.status === "approved" ? "bg-green-500/30 text-green-300" :
                                  step.status === "rejected" ? "bg-red-500/30 text-red-300" :
                                  i === 0 ? "bg-blue-500/30 text-blue-300" : "bg-white/10 text-gray-400"}`}>
                                {step.status === "approved" ? <Icons.Check /> :
                                 step.status === "rejected" ? <Icons.X /> :
                                 step.step_order}
                              </div>
                              <div className="flex-1 min-w-0">
                                <span className="text-sm text-gray-200">{step.approver_name}</span>
                                {step.approver_role && (
                                  <span className="text-xs text-gray-500 ml-2">{step.approver_role}</span>
                                )}
                                {step.comment && (
                    ...
                    <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-3 mb-5 flex gap-2 text-xs text-blue-300">
                      <Icons.Info />
                      <span>HITL設計: この操作は最終的な人間の判断です。AIの提案を参考にしつつ、ご自身の判断で承認・差戻しを行ってください。</span>
                    </div>

                    <div className="flex gap-3">
                      <button
                        onClick={handleApprove}
                        className="flex-1 bg-green-600 hover:bg-green-500 text-white font-medium py-2.5 rounded-lg transition-colors text-sm flex items-center justify-center gap-2"
                      >
                        <Icons.Check />承認する
                      </button>
                      <button
                        onClick={handleReject}
                        className="flex-1 bg-red-600/80 hover:bg-red-600 text-white font-medium py-2.5 rounded-lg transition-colors text-sm flex items-center justify-center gap-2"
                      >
                        <Icons.X />差戻す
                      </button>
                    </div>
        ...
        {tab === "notifications" && (
          <div className="space-y-2">
            {notifications.length === 0 ? (
              ...
              notifications.map(n => (
                <div key={n.id} className={`border rounded-xl p-4 transition-colors
                  ${n.is_read ? "bg-white/3 border-white/5 opacity-50" : "bg-white/6 border-white/15"}`}>
          ...
          <div className="max-w-2xl space-y-5">
              ...
              <div>
                ...
                >
                  {Object.entries(TYPE_LABEL).map(([k, v]) => (
                    <option key={k} value={k} className="bg-[#1a1d27]">{v}</option>
                  ))}
                </select>
              </div>
              <div>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <button
                  onClick={() => setForm({ ...form, approvers: [...form.approvers, { id: "", name: "", role: "" }] })}
              ...
              <div className="space-y-2">
                {form.approvers.map((a, i) => (
                  <div key={i} className="grid grid-cols-3 gap-2">
                    <input
                      value={a.id}
                      onChange={e => { const u = [...form.approvers]; u[i] = { ...a, id: e.target.value }; setForm({ ...form, approvers: u }); }}
                      className={inputCls}
                      placeholder="ユーザーID"
                    />
                    <input
                      value={a.name}
                      onChange={e => { const u = [...form.approvers]; u[i] = { ...a, name: e.target.value }; setForm({ ...form, approvers: u }); }}
                      className={inputCls}
                      placeholder="氏名"
                    />
                    <input
                      value={a.role}
                      onChange={e => { const u = [...form.approvers]; u[i] = { ...a, role: e.target.value }; setForm({ ...form, approvers: u }); }}
                      className={inputCls}
                      placeholder="役職"
                    />
                  </div>
            ...
            <button
              ...
              className="w-full bg-blue-600 hover:bg-blue-500 text-white font-medium py-3 rounded-xl transition-colors text-sm flex items-center justify-center gap-2"
            >
              <Icons.Plus />申請を提出する
            </button>
          </div>
        ...
        {tab === "jsox" && (
          <div className="max-w-2xl space-y-5">
            ...
            <div>
              ...
              <input
                value={jsoxForm.process_name}
                onChange={e => setJsoxForm({ ...jsoxForm, process_name: e.target.value })}
                className={inputCls}
                placeholder="例: 売上計上プロセス、購買・支払プロセス..."
              />
            </div>
            ...
            <div>
              ...
              <textarea
                value={jsoxForm.process_description}
                onChange={e => setJsoxForm({ ...jsoxForm, process_description: e.target.value })}
                className={`${inputCls} resize-none`}
                rows={4}
                placeholder="業務の流れ・担当者・使用システムなどを記載..."
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  value={jsoxForm.fiscal_year}
                  onChange={e => setJsoxForm({ ...jsoxForm, fiscal_year: e.target.value })}
                  className={inputCls}
                />
              </div>
              <div>
                ...
                <input
                  value={jsoxForm.risk_focus}
                  onChange={e => setJsoxForm({ ...jsoxForm, risk_focus: e.target.value })}
                  className={inputCls}
                  placeholder="例: 不正リスク, IT全般統制"
                />
              </div>
            </div>
            ...
            <button
              onClick={handleJSOX}
              disabled={jsoxLoading}
              className="w-full bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-medium py-3 rounded-xl transition-colors text-sm flex items-center justify-center gap-2"
            >
              <Icons.FileText />
              {jsoxLoading ? "生成中..." : "3点セットを生成する"}
            </button>
